...
//
// Viewport = Clip + Extrinsic + Expand + Grow(1) + NoAnimate — the usual
// “fill remaining space and scroll” recipe from step 09 in one word.
//
//	go run . --png out.png
package main
...
var messages = []msg{
	...
	{"casey", "Step 11: Grow+Clip alone broke compose.", "10:07"},
	{"alex", "Step 11: Extrinsic fixed the budget.", "10:08"},
	{"devon", "Step 11: Viewport is that recipe in one helper.", "10:09"},
	{"blair", "Use Viewport for nested fill-and-scroll panes.", "10:10"},
	{"casey", "Not as an outer app root — the engine already has one.", "10:11"},
	{"alex", "Members still empty until the next step.", "10:12"},
	{"devon", "Try resizing the live window interactively.", "10:13"},
	{"blair", "Fixed-width rails do not steal center space.", "10:14"},
	{"casey", "Compose stays pinned. Scroll the middle.", "10:15"},
}
...
func main() {
	...
	app.SetupWindow("Layout shell — step 11 (Viewport)", winW, winH)
	app.Run(frame)
}

func frame() {
	Container(Attrs(Expand, FixHeight(48), Background(280, 45, 42, 1), Center), func() {
		Label("top bar · step 11 Viewport", FontSize(16), FontWeight(WeightSemibold), TextColor(0, 0, 100, 1))
	})
	...
	Container(Attrs(Row, Grow(1), Expand), func() {
		...
		Container(Attrs(Row, Grow(1), Expand), func() {
			...
			Container(Attrs(Grow(1), Expand, Background(200, 20, 78, 1)), func() {
				...
				// Same as step 09's Extrinsic recipe, via Viewport.
