...
//
// Messages use Grow+Expand+Clip only. Content height inflates the middle and
// the compose strip is half-cut or pushed off-screen. Fixed in step 10.
//
//	go run . --png out.png
package main
...
var messages = []msg{
	...
	{"alex", "With only Grow+Expand+Clip, long content wins.", "10:08"},
	{"devon", "The middle region sizes to its children, not the budget.", "10:09"},
	{"blair", "Compose should pin under the messages — but look…", "10:10"},
	{"casey", "…it gets half-cut or shoved past the window edge.", "10:11"},
	{"alex", "Step 10 adds Extrinsic so height comes from the parent.", "10:12"},
	{"devon", "Step 11 packages that recipe as Viewport.", "10:13"},
	{"blair", "Fixed-width rails do not steal center space.", "10:14"},
	{"casey", "That's the bug this step is meant to show.", "10:15"},
}
...
func main() {
	...
	app.SetupWindow("Layout shell — step 09 (broken compose)", winW, winH)
	app.Run(frame)
}

func frame() {
	Container(Attrs(Expand, FixHeight(48), Background(280, 45, 42, 1), Center), func() {
		Label("top bar · step 09 intentional bug", FontSize(16), FontWeight(WeightSemibold), TextColor(0, 0, 100, 1))
	})
	...
	Container(Attrs(Row, Grow(1), Expand), func() {
		...
		Container(Attrs(Row, Grow(1), Expand), func() {
			...
			// Message content sets the middle height → compose is clipped.
