package main

import (
	"go.hasen.dev/shirei/app"

	. "go.hasen.dev/shirei"
	. "go.hasen.dev/shirei/widgets"
)

func main() {
	app.SetupWindow("Todo List Demo", 400, 400)
	app.Run(appView)
}

type TodoItem struct {
	Text string
	Done bool
}

var todos = []TodoItem{
	{Text: "Go to the store"},
	{Text: "Buy some milk"},
}
var nextItem string

func appView() {
	ModAttrs(Pad(10), Gap(10))
	Container(Attrs(Row, Gap(10), CrossAlign(AlignMiddle)), func() {
		TextInput(&nextItem)
		var enterKey = HasFocusWithin() && GetFrameInput().Key == KeyEnter
		var btnClick = Button(NoIcon, "Add")
		if enterKey || btnClick {
			todos = append(todos, TodoItem{Text: nextItem})
			nextItem = ""
		}
	})
	for i := range todos {
		item := &todos[i]
		Container(Attrs(Row, Pad(10), Gap(10), Corners(2), CrossAlign(AlignMiddle)), func() {
			var clr = Vec4{0, 0, 0, 1}
			var style = StyleNormal
			var bullet = SymBox
			if item.Done {
				clr[3] = 0.5
				style = StyleItalic
				bullet = SymBoxTick
			}
			Icon(bullet, FontSize(20))
			Label(item.Text, FontSize(16), TextColorVec(clr), FontStyle(style))
			if PressAction() {
				item.Done = !item.Done
			}
		})
	}
}
