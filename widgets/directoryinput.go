...
func DirectoryInput(text *string) {
	...
	Layout(TW(Row, CrossMid), func() {
		...
		if stat.Exists {
			if CtrlButton(SymFolder, "Open Directory", true) {
	...
	var menuHidden = Use[bool]("menu-hidden")
	...
	if focused {
		...
		if *menuHidden && IdIsClicked(editorId) && !IdReceivedFocusNow(editorId) {
			*menuHidden = false
		}
		...
		if !*menuHidden && len(suggest.items) > 0 {
			var editorRD = GetRenderDataOf(editorId)
			pos := editorRD.ResolvedOrigin
			size := editorRD.ResolvedSize
			pos[1] += size[1] + 4
			Popup(func() {
				Layout(TW(Clip, FloatV(pos), BG(0, 0, 100, 1), BR(4), MaxHeight(100), Spacing(2), BW(1), Bo(0, 0, 0, 0.5)), func() {
					ScrollOnInput()
