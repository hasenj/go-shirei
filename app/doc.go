...
// ALSA via purego on linux, winmm waveOut on windows). iOS uses Playback +
// MixWithOthers (audible with silent switch on, mixes with other apps; no
// background audio) and reports interruptions via
// shirei.GetInputState().AudioInterrupted. Each OS's audio backend uses the same
// linking mechanism its window backend already relies on (cgo on Apple
// platforms, no cgo on linux/windows).
package app
