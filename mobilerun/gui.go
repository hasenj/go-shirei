...
func RootView() {
	...
	Container(Attrs(Pad2(10, 20), Gap(4)), func() {
		...
		Container(Attrs(Gap(6)), func() {
			...
			Container(Attrs(Row, CrossMid, Gap(8)), func() {
				...
				case pkgsErr != nil && len(pkgs) == 0:
					MenuButtonExt(current, ButtonAttrs{Icon: TypArrowSortedDown, Disabled: true}, func() {})
				case !pkgsReady && len(pkgs) == 0:
					MenuButtonExt("Scanning for package main…", ButtonAttrs{Icon: TypArrowSortedDown, Disabled: true}, func() {})
				case pkgsReady && len(pkgs) == 0:
					MenuButtonExt("No package main found", ButtonAttrs{Icon: TypArrowSortedDown, Disabled: true}, func() {})
				default:
				...
				if ButtonExt("", ButtonAttrs{Icon: SymRefresh, Disabled: runner.IsBusy()}) {
					invalidatePackages()
			...
			Label("package main under this directory (or go.work modules).",
		...
		Container(Attrs(Row, CrossMid, Gap(10)), func() {
			...
			if ButtonExt(runLabel, ButtonAttrs{Accent: AccentMeadow, Disabled: busy}) {
				go persistAndRun()
			}
			if cfg.Platform == platformAndroid {
				if ButtonExt("Screencap", ButtonAttrs{Disabled: busy}) {
					go grabScreencap()
				}
			}
		})
