...
func TestExpandOrientationLandscape(t *testing.T) {
	r, err := expandCapabilities([]string{"orientation-landscape", "camera"}, nil)
	if err != nil {
		t.Fatal(err)
	}
	if r.Orientation != "landscape" {
		t.Fatalf("orientation = %q, want landscape", r.Orientation)
	}
}
